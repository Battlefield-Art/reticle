...
**Touching `reticle init`, `@reticlehq/vite-plugin`, `@reticlehq/next`, or anything a user runs before their first session?** The gates in this repo cannot see it. Every app in `apps/` is already instrumented, so re-running `init` over one reports "already wired" for every step and proves nothing — which is exactly how v2.3.0 shipped a Next.js install that connected **0% of the time** through three independent defects, none of which any check short of opening a browser could see. The install gate lives in the sibling **`reticle-fixtures`** repo (`node scripts/verify.mjs`): it installs into pristine upstream apps on a `clean` branch, boots each one, and asserts a session actually appears. Read [`docs/fixtures.md`](docs/fixtures.md).
