...
function render(entries) {
  ...
  const lines = [
    ...
    `| ${MARK[Verdict.RUNNABLE_UNVERIFIED]} | \`init\` wrote a runnable entry where the client documents it — **nobody has run that client** |`,
    ...
    for (const { record } of forVersion.sort((a, b) =>
      ...
      lines.push(
        `| ${record.client.id}${record.client.version ? ` ${record.client.version}` : ''} ` +
          `| ${mark} ${record.verdict} | ${record.checks?.toolsVisible ?? '—'} | ${record.host?.os ?? '—'} ` +
          `| ${record.by ?? '—'} |`,
      );
    }
    lines.push('');
