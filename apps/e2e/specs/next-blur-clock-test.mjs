...
await T('reticle_act', { ref: field, action: 'fill', args: { value: 'Hello Reticle' } });
const since = (await T('reticle_act', { ref: field, action: 'blur' })).since;
const committed = await T('reticle_assert', {
  timeout_ms: 2000,
  predicate: { kind: 'allOf', predicates: [
    { kind: 'signal', name: 'field:committed', dataMatches: { value: 'Hello Reticle' }, since },
    { kind: 'text', contains: 'Committed: Hello Reticle', visible: true },
  ]},
});
...
await T('reticle_clock', { freeze: true });
await T('reticle_act', { ref: await refOf('testid', 'show-toast'), action: 'click' });
const shown = await T('reticle_assert', {
  timeout_ms: 2000,
...
await T('reticle_clock', { advanceMs: 4000 });
await sleep(250);
const gone = await T('reticle_assert', {
  timeout_ms: 2000,
...
await T('reticle_clock', { reset: true });
