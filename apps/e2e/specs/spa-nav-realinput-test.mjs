...
// baseline on "/"
let ref=await refOf();
const a1=await T('reticle_act',{ref,action:'click',args:{native:true}});
chk('pre-nav: reticle_act is REAL', a1.inputMode==='real', `inputMode=${a1.inputMode} url=${sess.url}`);
...
ref=await refOf();
const a2=await T('reticle_act',{ref,action:'click',args:{native:true}});
chk('after pushState: reticle_act is STILL REAL (was synthetic before the fix)', a2.inputMode==='real', `inputMode=${a2.inputMode}`);
