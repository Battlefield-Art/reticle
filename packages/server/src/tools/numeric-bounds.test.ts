...
describe('numeric bound helpers', () => {
  it('depth refuses a negative (the reported silent-ignore) and keeps 0 (collapse-to-marker)', () => {
    expect(depthSchema.safeParse(-5).success).toBe(false);
    expect(depthSchema.safeParse(0).success).toBe(true);
  ...
  it('wait-ready: 0 is poll-once; a hang-length wait is refused', () => {
    expect(waitReadyMsSchema.safeParse(0).success).toBe(true);
    expect(waitReadyMsSchema.safeParse(-1).success).toBe(false);
    expect(waitReadyMsSchema.safeParse(MAX_WAIT_READY_MS).success).toBe(true);
    expect(waitReadyMsSchema.safeParse(MAX_WAIT_READY_MS + 1).success).toBe(false);
  });
});
...
describe('the reported silent-ignore cases are refusals on the tool schema', () => {
  ...
  it('reticle_wait_ready { timeoutMs: -1 } is refused, not a silent clamp; 0 still means poll once', () => {
    const shape = inputShape(ReticleTool.WAIT_READY);
    expect(parseField(shape, 'timeoutMs', -1).success).toBe(false);
    expect(parseField(shape, 'timeoutMs', 0).success).toBe(true);
    expect(parseField(shape, 'timeoutMs', 1e9).success).toBe(false);
  });
});
...
describe('every top-level numeric tool input has a bound', () => {
  it('refuses an unbounded z.number() on any handler input, including retired tools', () => {
    // `reticle_wait_ready` is retired from the advertised surface, so a lock over TOOLS
    // alone would miss timeoutMs the same way depth: -5 shipped: accepted, then clamped.
