...
/** `capDepth` with 32 levels is already past any store an agent can read. */
...
/**
 * Store projection depth. 0 collapses this node to a size marker (a real, tested call).
 * Negative is "no cap" inside `capDepth`, which is how `depth: -5` silently returned the full store.
 */
export const depthSchema = z.number().finite().int().nonnegative().max(MAX_STATE_DEPTH);
...
/**
 * Wait-for-session. 0 is "poll once" (the not-ready test). 30s is already a missing app;
 * a two-minute hang at init is not a bound, it is a stuck agent.
 */
export const MAX_WAIT_READY_MS = 30_000;
export const waitReadyMsSchema = z.number().finite().int().nonnegative().max(MAX_WAIT_READY_MS);
