...
describe('statusNextAction', () => {
  it('says an agent starts the daemon, when no daemon is running', () => {
    ...
    const next = statusNextAction({ running: false, sessionCount: 0, previouslyConnected: false });
  ...
  it('does not send a previously-connected project back to `init`', () => {
    ...
    const next = statusNextAction({ running: false, sessionCount: 0, previouslyConnected: true });
    expect(next).not.toContain('init');
  });

  it('names the missing piece when the daemon is up and nothing has connected', () => {
    const next = statusNextAction({ running: true, sessionCount: 0, previouslyConnected: false });
    expect(next).toBeDefined();
    expect(next).not.toBe('');
  });

  it('says the wiring is fine and the tab is missing, when an app connected here before', () => {
    const next = statusNextAction({ running: true, sessionCount: 0, previouslyConnected: true });
  ...
  it('stays quiet when a session IS connected — there is nothing to fix', () => {
    ...
    expect(
      statusNextAction({ running: true, sessionCount: 1, previouslyConnected: true }),
    ).toBeUndefined();
  });
});
