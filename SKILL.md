...
**Empty list?** Read the `why` field first; the daemon can see whether a session was ever here and whether a dev server is listening. Then, in order: is the SDK imported and called in the app entry, is the dev server actually serving that entry, is the connect guarded on `hostname === 'localhost'`, and is the bridge port the same number on both sides. **Do not tell the user to start a dev server they are already running.** Full checklist: `https://docs.reticle.sh/troubleshooting.md`.
...
The surface is deliberately small: `default` 19, `all` 48. `reticle_tools` loads the argument grammar for the rest on demand, and `reticle_run` invokes any of them by name.
