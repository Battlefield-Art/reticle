...
- **Tool profile (default `hybrid`).** By default Reticle advertises the 14 core tools directly (sessions/navigate/snapshot/query/act/act_and_wait/observe/network/console/wait_for/assert/state/inspect/feedback — the whole detect loop, plus the file-pointer and the feedback channel) PLUS two meta-tools that keep every other tool one call away: `reticle_tools` (discover — no args lists every tool name + summary; `names:[…]` loads full params on demand) and `reticle_run({ tool, args })` (invoke any tool by name). So to record/replay/verify a flow under the default, call `reticle_run({ tool:"reticle_flow_verify", args:{…} })` (or `reticle_tools` first to see params). Want every tool advertised directly (no `reticle_run` indirection)? set `RETICLE_TOOL_PROFILE=full`. `=dynamic` goes the other way and advertises only the 2 meta-tools. There are three profiles, because there were only ever three distinct surfaces: `core` was byte-identical to `hybrid`, and `standard` paid a permanent per-turn cost to advertise tools `reticle_run` already reaches — both names are retired, still resolve to `hybrid`, and the daemon reports that they did. **Sizes are deliberately not quoted here** — a count in prose goes stale and has three times already. `reticle_tools` reports the live surface, and SKILL.md carries the one gated table.
