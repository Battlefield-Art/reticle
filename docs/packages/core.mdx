...
`@reticlehq/core` is the bottom of the dependency graph. Every message that crosses a boundary, browser to bridge, bridge to agent, is defined here as a named constant plus a zod schema.

**Version 2.8.0. Apache 2.0. Depends on `zod` and nothing else.**
...
| `./desktop-contract` | A CommonJS shim, generated at build, for the five strings the desktop adapters share |
...
`src/index.ts` is a pure barrel: 26 `export *` re-exports plus one named export. Grouped by what they are for:
...
`EventType`, `ActionType`, `ElementState`, `QueryBy`, `MessageKind`, `ReticleCommand`, `PhenomenonType`, `SettleReason`, `ComponentStateReason`, `ComponentStateResult`, `InputMode`, `InputModeReason`, `ActionWarning`, `DriveErrorCode`, `TruncationChannel`, `EventAttribution`, `PerfMetric`, `PresenterMode`, `SnapshotMode`, `VisualReason`, `CONSOLE_LEVELS`, `Verified`.
...
```ts
import { DESKTOP_CONTRACT, RETICLE_IPC_GLOBAL } from '@reticlehq/core/desktop-contract';
```

Five strings shared by the Electron preload and the Tauri crate, in CommonJS because a preload script is required before any ESM transform:
...
| `RETICLE_FULL_PAGE_UNSUPPORTED` | The `VisualReason` value for a refused full-page capture |
