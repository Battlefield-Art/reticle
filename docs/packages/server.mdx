...
**Version 2.7.0. Licensed under `SEE LICENSE IN LICENSE`, not Apache 2.0 like the rest.** This is the package the enterprise terms apply to.
...
The `bin` entry is `reticle`, so `npx reticle <command>` works once it is on disk.
