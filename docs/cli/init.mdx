...
```bash
npx reticle init [--dry-run] [--port N] [--no-mcp] [--no-install] [--app <dir>] [--yes]
```
...
reticle init (dry run — no files written)
  ...
  [✓] Install dependencies → package.json
      pnpm add -D @reticlehq/react@2.7.0 @reticlehq/vite-plugin@2.7.0
...
```bash
cd apps/web
npx reticle init --dry-run     # read the plan first
npx reticle init               # then run it
npx reticle status             # confirm the app actually connected
```
