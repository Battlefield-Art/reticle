...
<Warning>
  ...
  — and nothing is written to the proxy log, because the process that writes it is the one that
...
```bash
npx reticle kill            # free 4400
npx reticle status          # confirm
```
