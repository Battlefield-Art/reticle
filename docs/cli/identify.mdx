...
```bash
npx reticle identify --context <company|side_project|open_source|learning> [--company "Acme"] [--email you@acme.com] [--forget]
```
...
```bash
npx reticle identify --context open_source --email maintainer@example.com
npx reticle identify --forget
```
