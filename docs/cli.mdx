...
The `reticle` CLI wires Reticle into a project (`init`), runs and diagnoses the daemon your app connects to (`serve`, `status`, `doctor`, `kill`, `restart`), and gates a build on verification in CI (`verify`, `affected`, `gate`). This page is the index; each command has its own page with real captured output and measured exit codes.
...
<Warning>
  `reticle drive` and `reticle verify` both bind the bridge port themselves, so either will die with
  a raw `Error: listen EADDRINUSE: address already in use 127.0.0.1:4400` stack trace if a daemon is
  already listening. Which is the normal state once your agent has started one. `reticle mcp` reuses
  a running daemon; those two do not. Stop the daemon first, or point them elsewhere with
  `RETICLE_PORT`.
</Warning>
