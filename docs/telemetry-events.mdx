...
`isSessionScoped(kind)` decides whether `sessionId` is attached. Ten kinds are session-scoped:

`daemon_started`, `daemon_stopped`, `session_progress`, `mcp_client_connected`, `app_instrumented`, `project_profiled`, `verification_completed`, `bug_found`, `runtime_crashed`, `feedback_submitted`.
...
Fires on the first ever run on a machine.
...
| `attribution` | `BugAttribution`, optional | `app`, `request` or `reticle`. **Omitted rather than guessed** when the evidence cannot say |
...
- `attribution` separates a defect in the app under test from the agent's own bad predicate and from Reticle's own blind spot. Only `attribution: 'app'` belongs in a published defect count.
