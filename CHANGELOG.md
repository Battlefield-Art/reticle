...
- **The predicate grammar was re-sent six times per turn.** 211 B describing `{ kind, ...fields }` rode on six advertised tools — 1,266 B, 23% of all parameter prose, paid on every turn to state one thing six times. It is now spelled out once and cross-referenced, cutting the hybrid surface from 14,104 B to 13,314 B (~197 tok/turn) with nothing lost: the full grammar is still in context every turn. The advertised config also moved behind one exported builder so its tests assert on the REAL advertised text — the earlier abbreviation guard read raw tool definitions, which are trimmed later, and would have passed with the bug fully present. (`@reticlehq/server`)
