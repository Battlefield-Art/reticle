...
- **The SDK no longer refuses to start inside a desktop app.** The localhost gate exists to stop a remote website driving a developer's local bridge, but it read a `file://`, `tauri://` or `http://tauri.localhost` page as remote and blocked the connection outright. A local desktop webview is now recognized as local; a remote page reaching a loopback bridge is still blocked, and a remote *bridge* still requires the explicit opt-in plus a token. (#64) (`@reticlehq/browser`, `@reticlehq/core`)
