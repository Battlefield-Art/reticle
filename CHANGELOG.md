...
> ### Read this first: some verdicts that came back `no` or `unknown` will now pass
>
> Three fixes below stop a verdict being decided by a channel the caller never asked about. Each one turns a red or an `unknown` on a **correct** app into the green it should always have been, so a check that used to fail may now pass, and a suite that treats "this assertion has never gone green" as an expected state will change behaviour. Nothing was loosened: every rule involved still catches the false green it was built for, and each one has a test that must stay red to prove it. If you are relying on one of these verdicts staying negative, the negative was the bug.
