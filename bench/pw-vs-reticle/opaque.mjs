...
const SAMPLE = [
  ...
  },
  {
    ...
    kind: 'state',
    statePath: 'deployments.0.service',
    text: 'Generate',
    nav: 'Compose',
    expect: 'reticle-only',
  },
...
async function reticleRun() {
  ...
  const chrome = spawn(
    ...
    [
      ...
      '--disable-gpu',
      '--no-first-run',
      `--user-data-dir=${path.join(os.tmpdir(), 'opq-' + process.pid)}`,
      ORIGIN,
    ],
  ...
  const goto = async (u) => {
    await call('reticle_navigate', { sessionId: sid, url: u });
    for (let i = 0; i < 30; i++) {
      const s = await call('reticle_sessions', {});
      const f = (s?.sessions ?? []).find((x) => x.url === u) ?? s?.sessions?.[0];
      if (f) {
        sid = f.sessionId;
        if (i > 1) break;
      }
  ...
  const byText = async (t) =>
    (await call('reticle_query', { sessionId: sid, by: 'text', value: t }))?.elements?.[0]?.ref;
  const clickText = async (t) => {
    const r = await byText(t);
    if (r)
  ...
  for (const b of SAMPLE) {
    ...
    try {
      ...
      if (b.kind === 'usable') {
        ...
        caught =
          !!ins &&
          (ins.styles?.opacity === '0' ||
            ins.occluded === true ||
            (ins.box && (ins.box.width === 0 || ins.box.height === 0)));
        note = ins
          ? `opacity=${ins.styles?.opacity} occluded=${ins.occluded}`
          : 'not found by text';
      } else if (b.kind === 'console') {
        const act = await (async () => {
          const r = await byText('benchmark note');
          return r;
        })(); // fill prompt: opaque strips testid, fill by nearest textbox
        ...
        caught = (con?.logs ?? []).length > 0;
        note = `errors=${(con?.logs ?? []).length}`;
...
async function playwrightRun() {
  ...
  for (const b of SAMPLE) {
    ...
    let caught = false,
      note = '';
    try {
      await page.goto(url(b.id), { waitUntil: 'domcontentloaded' });
      await sleep(700);
      await page
      ...
      if (b.kind === 'usable') {
        ...
        errs.length = 0;
        await page
          .getByText(b.text, { exact: false })
          .first()
          .click({ force: true })
          .catch(() => {});
        await sleep(500);
        caught = errs.length > 0;
        note = `errors=${errs.length}`;
      } else if (b.kind === 'state') {
        await page
          .getByText(b.text, { exact: false })
          .first()
        ...
        await sleep(400);
        caught = false;
        note = 'no app-state access — store invariant unreadable from an opaque DOM';
      }
    } catch (e) {
      note = 'ERR ' + e.message;
    }
...
(async () => {
  await ensureApp();
  await sleep(1000);
  console.log('opaque=2 — reticle…');
  const r = await reticleRun();
  console.log('opaque=2 — playwright…');
  const p = await playwrightRun();
  const rows = [...r, ...p];
  ...
  console.log('|---|---|:--:|:--:|');
  for (const b of SAMPLE) {
    const rr = r.find((x) => x.bug === b.id),
      pp = p.find((x) => x.bug === b.id);
    console.log(
      `| ${b.id} | ${b.expect} | ${rr.caught ? '✅' : '⬜'} ${rr.note.slice(0, 30)} | ${pp.caught ? '✅' : '⬜'} ${pp.note.slice(0, 30)} |`,
    );
  }
  process.exit(0);
})().catch((e) => {
  console.error('OPAQUE ERR', e);
  process.exit(1);
});
