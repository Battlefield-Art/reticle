...
What are the tradeoffs in **token usage, verification latency, regression-detection capability, and developer effort** across three browser-verification strategies an AI coding agent can use:
...
Each scenario file (`scenarios/<id>.json`) defines: `setup`, `injected_regression`, `expected_behavior`, `failure_signal`, `success_criteria`, `canonical_task` (the NL prompt), and per-tool `recipe` (idiomatic call sequence for Layer A).
...
Scenario #5 is deliberately the pilot because a snapshot-only verification _passes_ (UI intact) while the regression is real — the sharpest test of the observability hypothesis.
