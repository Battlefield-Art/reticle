// W11 §5 — ORACLE GUARDS: the suite that tests Reticle's own judgment.
//
...
let seq = 0;
const ev = (type, data = {}, t = ++seq, extra = {}) => ({ t, seq, type, sessionId: 'guard', data, ...extra });
...
guard('settled-vs-inflight', 'a request in flight must not read as settled, however quiet the DOM is', () => {
...
guard('stale-signal-behind-cursor', 'a signal from BEFORE the action must not satisfy the assertion', () => {
...
guard('blindspot-forces-partial-coverage', 'an unobservable region must degrade coverage, never stay silent', () => {
  const window = [ev(EventType.BLIND_SPOT, { kind: BlindSpotKind.CROSS_ORIGIN_IFRAME, count: 1 })];
...
guard('truncation-dirties-integrity', 'dropped evidence must mark the verdict, not be silently absorbed', () => {
...
guard('presence-only-cannot-clear-a-net-bar', 'a presence-graded green must not satisfy a consequence bar', () => {
...
guard('churn-cannot-evict-scarce-evidence', 'a DOM flood must not push the one failed request out of the window', () => {
  const buf = new RingBuffer({ maxEvents: 50 });
  buf.push(ev(EventType.NET_REQUEST, { url: '/api/save', status: 500, ok: false }, 1), 1);
  for (let i = 0; i < 400; i++) buf.push(ev(EventType.DOM_TEXT, { text: String(i) }, 2 + i), 2 + i);
  return buf.since(0).some((e) => e.type === EventType.NET_REQUEST);
});

guard('learned-ambient-region-still-settles', 'a churning feed must not block settle once learned ambient', () => {
  // A feed appends a NEW element each tick and removals carry no ref — so the key must be the region.
  const churn = [];
  for (let i = 0; i < 4; i++) {
    churn.push(ev(EventType.DOM_ADDED, { region: 'feed' }, 990 + i, { ref: `e${String(800 + i)}` }));
...
guard('clean-build-zero-findings', 'a clean window must raise nothing — no crying wolf', () => {
  ...
  const honesty = buildHonestyBlock({ grade: 'signal', coveragePartial: coverage.coverage === 'partial' });
  return (
...
for (const g of guards) {
  ...
  console.log(`  ${ok ? '✓' : '✗'} ${g.name.padEnd(36)} ${g.why}${err === undefined ? '' : ` [threw: ${err}]`}`);
}
console.log(
