...
- **The MCP schema tax, paid on every request before any work.** Measured live, all servers one run: our default is **6,559 tok** vs playwright-mcp 3,725 and chrome-devtools-mcp 5,116 — still **1.76× the incumbent**, down from 2.63× after advertising a compact predicate. Our `dynamic` profile is 282 tok, 13× cheaper than Playwright, but it is not the default. Artifact: `bench/raw/schema-tax.json`.
