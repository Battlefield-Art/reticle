...
function renderManual(rows) {
  ...
  const lines = [
    ...
    '',
    'Every row below is a call that was made against a running app and the answer that came back.',
    'Nothing here is described from the source or from intent. If a claim has no probe behind it,',
    'it is not in this file. That is the whole point: the hand-written guides drifted, and the',
    'drift was invisible because nobody ran them.',
    '',
    '`refused (names the fix)` means the error told you what to call instead, so you can correct',
    'without reading anything else. `refused (no fix named)` means it did not.',
    '',
  ];
