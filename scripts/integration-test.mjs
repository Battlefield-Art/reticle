 ...
 * Prerequisites:
 *   pnpm build
 *   RETICLE_PORT=58432 pnpm --filter @reticlehq/bench-app dev   (in a separate terminal)
...
async function main() {
  ...
  if (!(await probeTcp(RETICLE_PORT))) {
    fail(
      'Daemon',
      `nothing on port ${RETICLE_PORT} — start with: RETICLE_PORT=${RETICLE_PORT} pnpm --filter @reticlehq/bench-app dev`,
    );
    process.exit(1);
  }
  ...
  if (!demoUrl) {
    ...
    warn(`  RETICLE_PORT=${RETICLE_PORT} pnpm --filter @reticlehq/bench-app dev`);
